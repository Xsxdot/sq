...
func TestResetCursorRewindsAndClearsInflight(t *testing.T) {
	...
	got, err := f.dl.Receive(context.Background(), "g", "t-reset", 0, 2, time.Minute, 0, AllPass)
	if err != nil || len(got) != 2 {
		t.Fatalf("首轮应收 2 条: %d %v", len(got), err)
	}
	// 未 ack 期间正常路径收不到任何东西（都在 inflight 里且未过期）
	if got, _ := f.dl.Receive(context.Background(), "g", "t-reset", 0, 2, time.Minute, 0, AllPass); len(got) != 0 {
...
func TestResetCursorForwardSkips(t *testing.T) {
	...
	got, err := f.dl.Receive(context.Background(), "g", "t-skip", 0, 2, time.Minute, 0, AllPass)
