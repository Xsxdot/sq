...
**执行基线：** `v4flash-sq-m1` 工作树的 M1 实现（HEAD `696a388`，含 M1 审阅修复：BodyDigest/BodyEncoding、4MB 常量统一、死代码清理）。本计划在该分支（或其合入 main 后的主干）上执行；开工前先 `make test && make e2e` 确认基线全绿。
...
internal/
  config/config.go            修改：新增 default_max_attempts / retention_check_interval /
                                    disk_watermark_percent + 既有项校验补全
  ...
  rpc/
    server.go                 修改：Settings 回发补 BackoffPolicy；New 增 writeBlocked 参数
    receive.go                修改：TAG 表达式解析接入；toPBMessage 补 InvisibleDuration
    send.go                   修改：磁盘水位拒写（FORBIDDEN）
cmd/sq/main.go                修改：retention 任务装配与停机顺序、水位 guard 装配
integration/rocketmq_sdk_test.go  修改：Tag 过滤 e2e、DLQ e2e
...
### Task 1: M1 审阅收尾（config 校验、Settings 退避默认值、InvisibleDuration 回填）

M1 审阅遗留的三个小项，先清干净再开新功能。

**Files:**
- Modify: `internal/config/config.go`
- Modify: `internal/rpc/server.go`（`fillServerDefaults`）
- Modify: `internal/rpc/receive.go`（`toPBMessage`）
- Test: `internal/config/config_test.go`、`internal/rpc/server_test.go`、`internal/rpc/receive_test.go`

**Interfaces:**
- Consumes: M1 的 `config.Load`、`fillServerDefaults`、`toPBMessage`
- Produces: 行为收紧，无签名变化

- [ ] **Step 1: 写失败测试**

`internal/config/config_test.go` 追加（import 需补 `os`、`path/filepath`）：

```go
// TestLoadRejectsBadValues 配置校验：非法值必须在启动时报错，而非运行时才炸。
func TestLoadRejectsBadValues(t *testing.T) {
	cases := []struct{ name, yaml string }{
		{"queue_nums_zero", "default_queue_nums: 0\n"},
		{"bad_log_level", "log_level: verbose\n"},
	}
	for _, c := range cases {
...
`internal/rpc/server_test.go` 追加：

```go
// TestTelemetryEchoFillsBackoffPolicy 回包必须带重试退避默认值：
// 官方 SDK 会用 Settings.BackoffPolicy 覆盖本地退避策略，缺省回发会导致
// 客户端按零值退避（重试风暴）或行为未定义。
func TestTelemetryEchoFillsBackoffPolicy(t *testing.T) {
	c := newTestClient(t)
	...
	bp := resp.GetSettings().GetBackoffPolicy()
	if bp.GetMaxAttempts() == 0 || bp.GetExponentialBackoff() == nil {
		t.Fatalf("期望回包带指数退避默认值，得到 %v", bp)
	}
}
```
...
func TestToPBMessageSetsInvisibleDuration(t *testing.T) {
	s := &Server{}
	msg := s.toPBMessage(&core.Message{ID: "A", Topic: "t", Body: []byte("x")}, "g", 45*time.Second)
...
Run: `go test ./internal/config ./internal/rpc -run 'TestLoadRejectsBadValues|TestTelemetryEchoFillsBackoffPolicy|TestToPBMessageSetsInvisibleDuration' -v`
Expected: 三个测试 FAIL（校验缺失 / BackoffPolicy 为 nil / InvisibleDuration 为零值）

- [ ] **Step 3: 实现**

`internal/config/config.go` 的 `Load`，在 fsync 校验后追加：

```go
	if cfg.DefaultQueueNums == 0 {
		return nil, fmt.Errorf("配置 default_queue_nums 必须 >0")
	}
	switch cfg.LogLevel {
...
`internal/rpc/server.go` 的 `fillServerDefaults` 改为（import 补 `time`、`google.golang.org/protobuf/types/known/durationpb`）：

```go
// fillServerDefaults 在回发 Settings 前补上服务端口径的字段。
//
// SDK 客户端启动时会把自己带的 Settings 原样发给 broker，broker 回发时
// 会填充服务端侧默认值。若我们直接原样回显：
//   - Publishing.MaxBodySize 为 0 会让客户端拒发一切非空消息；
//   - BackoffPolicy 为 nil 会让客户端退避策略未定义（部分 SDK 按零值重试）。
func fillServerDefaults(st *pb.Settings) *pb.Settings {
	if st == nil {
		st = &pb.Settings{}
	}
	if st.GetBackoffPolicy() == nil {
		// 客户端 RPC 重试退避：100ms 起步 ×2，封顶 1s，最多 3 次。
		// 这是客户端侧行为参数，与服务端投递退避（deliver 的 retryBackoff）无关。
		st.BackoffPolicy = &pb.RetryPolicy{
			MaxAttempts: 3,
			Strategy: &pb.RetryPolicy_ExponentialBackoff{
				ExponentialBackoff: &pb.ExponentialBackoff{
					Initial:    durationpb.New(100 * time.Millisecond),
					Max:        durationpb.New(time.Second),
					Multiplier: 2,
				},
			},
		}
	}
	if p := st.GetPublishing(); p != nil {
		p.MaxBodySize = maxBodySizeBytes
	}
	return st
}
```

`internal/rpc/receive.go` 的 `toPBMessage`，`SystemProperties` 字面量中追加一行（import 补 `durationpb`）：
...
本 task 无新执行路径，核对两点即可：`fillServerDefaults` 的新注释说明了「为什么必须补默认值」（上码已含）；config 校验错误信息含非法值本身（上码已含）。

- [ ] **Step 6: Commit**

```bash
git add internal/config internal/rpc
git commit -m "fix: M1 审阅收尾（config 校验、Settings 退避默认值、InvisibleDuration 回填）"
```
...
- Modify: `cmd/sq/main.go:57`、`internal/core/produce/produce_test.go:18`、`internal/core/deliver/deliver_test.go:30`、`internal/rpc/server_test.go:29`（`meta.New` 增参，本 task 内全部更新）
...
`internal/config/config_test.go` 追加 case（并入 Task 1 的表驱动测试同函数或新函数均可）：
...
四个 `meta.New` 调用点更新：
- `cmd/sq/main.go:57` → `meta.New(st, cfg.AutoCreateTopic, cfg.DefaultQueueNums, cfg.DefaultMaxAttempts, logger)`
- `internal/core/produce/produce_test.go:18` → `meta.New(st, true, 4, 16, slog.Default())`
- `internal/core/deliver/deliver_test.go:30` → `meta.New(st, true, 1, 16, slog.Default())`
- `internal/rpc/server_test.go:29` → `meta.New(st, true, 4, 16, slog.Default())`
...
func TestAppendWritesKeyIndex(t *testing.T) {
	st, mt := newFixture(t) // 按本文件现有 fixture 写法取 st、mt；若 fixture 只返回 Producer，内联构造
	pr := New(st, mt, slog.Default())
...
（fixture 说明：`produce_test.go` 现有构造为 `store.Open(t.TempDir(),...)` + `meta.New(st, true, 4, 16, ...)` 内联写法；如无返回 `(st, mt)` 的 helper，就照现有测试的内联样式展开，**不得改动既有用例**。）
...
同时把本文件**全部既有 `f.dl.Receive(...)` 调用**补上第 8 个实参 `nil`（共 13 处，`TestReceiveAckFlow`/`TestUnackedRedeliveryAfterExpire`/`TestAckIdempotent`/`TestLongPollingWakesOnNewMessage`/`TestTwoGroupsIndependentCursors`/`TestMaxMsgsCapAcrossRedeliverAndFetch`/`TestOrphanInflightCleanedOnce`）。
...
```go
	...
	if v, ok, err := d.st.Get(store.CursorKey(group, topic, queueID)); err != nil {
		b.Close()
		return nil, fmt.Errorf("读取消费位点 (group=%s topic=%s q=%d): %w", group, topic, queueID, err)
	} else if ok {
		cursor = store.GetU64(v)
	}
	newCursor := cursor
	// budget 已被重投占满时不再扫描（M1 既有守卫，防 limit<=0 被 Scan 视为无限）
	if maxMsgs-len(out) > 0 {
		...
		if err != nil {
			b.Close()
			return nil, fmt.Errorf("扫描新消息 (group=%s topic=%s q=%d cursor=%d): %w", group, topic, queueID, cursor, err)
		}
		if newCursor > cursor {
			// 全部被过滤（out 为空）时也必须持久化位点，否则下趟重复扫同一批
...
（`dirty`/`b.Close()`/`Apply`/末尾 Debug 日志等骨架沿用 M1 现状，不重写。）
...
`receiveOnce` 签名：`receiveOnce(group, topic string, queueID uint32, maxMsgs int, invisible time.Duration, maxAttempts int32, filter *TagFilter)`。阶段 1 的重投循环整段替换为：
...
```go
	for _, r := range reds {
		...
		if err != nil {
			b.Close()
			return nil, fmt.Errorf("读取待重投消息 (group=%s topic=%s q=%d off=%d): %w", group, topic, queueID, r.offset, err)
		}
		if !ok {
			// 消息已被 retention 清理但 inflight 残留：清掉记录并跳过
		...
		if err != nil {
			b.Close()
			return nil, fmt.Errorf("解码待重投消息 (group=%s topic=%s q=%d off=%d): %w", group, topic, queueID, r.offset, err)
		}
		// 投递次数耗尽：转入死信 topic，不再投递。
		// DLQ 写入与 inflight 删除经 AppendWith 同批原子提交，与本函数的取件批次 b
		// 相互独立（此消息不进 b）。崩溃窗口内至多重复转入（at-least-once），
		// 死信消费端按消息 ID 幂等即可。
...
Run: `make test`
Expected: 全部 PASS。注意 `TestUnackedRedeliveryAfterExpire` 等既有用例只重投一次、断言 attempt=2，退避只影响 inflight 的过期时间不影响本次返回，故不受影响；若有用例依赖「第 2 次投递后快速再过期」需按 `TestExhaustedAttemptsGoToDLQ` 的方式注入小退避基数。
...
`cmd/sq/main.go` 的 `run()`：组件构造后、`net.Listen` 前插入（import 补 `sync`、`retention`）：
...
func TestSendMessageRejectedWhenDiskBlocked(t *testing.T) {
	c, blocked := newTestClientWithGuard(t) // server_test.go 的 guard 版 fixture
	blocked.Store(true)
	resp, err := c.SendMessage(context.Background(), &pb.SendMessageRequest{
		Messages: []*pb.Message{{
...
`internal/rpc/server_test.go`：`newTestClient` 改为调用新 fixture：

```go
// newTestClientWithGuard 同 newTestClient，另暴露磁盘水位开关。
func newTestClientWithGuard(t *testing.T) (pb.MessagingServiceClient, *atomic.Bool) {
	// ……与现 newTestClient 相同，仅：
	blocked := &atomic.Bool{}
	srv := New(cfg, mt, pr, dl, blocked, slog.Default())
	// ……其余相同，最后 return client, blocked
}

func newTestClient(t *testing.T) pb.MessagingServiceClient {
	c, _ := newTestClientWithGuard(t)
	return c
}
```
...
```go
	...
	// ……rpc 装配处：
	rpc.New(cfg, mt, pr, dl, writeBlocked, logger).Register(gs)
```
...
**Files:**
- Modify: `integration/rocketmq_sdk_test.go`（`writeConfig` + 两个新用例）
- Modify: `README.md`
- Test: `make e2e`

**Interfaces:**
- Consumes: 全部前置 task；官方 SDK `rmq_client.NewFilterExpression(tag)`（TAG 类型）
- 前置知识: e2e broker 是子进程，配置由 `writeConfig` 生成；`%DLQ%` 在 SDK topic 名合法字符集内

- [ ] **Step 1: 调整 e2e broker 配置**

`integration/rocketmq_sdk_test.go` 的 `writeConfig`：生成内容追加两行——`default_max_attempts: 2\n`（DLQ 用例 2 次投递即超限，控制时长）与 `disk_watermark_percent: 0\n`（e2e 机器磁盘状况不可控，关闭水位以免误拒写）。

- [ ] **Step 2: 写 Tag 过滤 e2e**

```go
// TestTagFilter 按 tag 订阅只收到匹配消息；不匹配的被服务端跳过。
func TestTagFilter(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		msgA := &rmq_client.Message{Topic: topic, Body: []byte(fmt.Sprintf("A-%d", i))}
		msgA.SetTag("tagA")
		if _, err := p.Send(context.TODO(), msgA); err != nil {
			t.Fatalf("send A: %v", err)
		}
		msgB := &rmq_client.Message{Topic: topic, Body: []byte(fmt.Sprintf("B-%d", i))}
		msgB.SetTag("tagB")
		if _, err := p.Send(context.TODO(), msgB); err != nil {
			t.Fatalf("send B: %v", err)
		}
	}

	// 消费者只订 tagA（newConsumer 的订阅表达式换为 NewFilterExpression）
	c, err := rmq_client.NewSimpleConsumer(&rmq_client.Config{
		Endpoint:      endpoint,
		ConsumerGroup: group,
		Credentials:   &credentials.SessionCredentials{AccessKey: "sq", AccessSecret: "sq"},
		...
		rmq_client.WithSimpleSubscriptionExpressions(map[string]*rmq_client.FilterExpression{
			topic: rmq_client.NewFilterExpression("tagA"),
		}))
	if err != nil {
	...
	t.Cleanup(func() { _ = c.GracefulStop() })

	got := receiveBodies(t, c, 4, 20*time.Second)
	if len(got) != 4 {
		t.Fatalf("tagA 消息数: %d (%v)", len(got), got)
	}
	for _, b := range got {
	...
	// 再轮询两整轮：tagB 已被位点跳过，不应再有任何投递
	if rest := receiveAllCycles(t, c, 2, 20*time.Second); len(rest) != 0 {
		t.Fatalf("被过滤消息泄漏: %v", rest)
	}
}
```

（import 需补 `fmt`、`strings`；若 SDK 无 `NewFilterExpression` 而是其他构造名，以 SDK `examples/` 实际 API 为准调整——预期内修正。）

- [ ] **Step 3: 写 DLQ e2e**

```go
// TestDLQ 不 ack 直到投递超限（default_max_attempts=2），死信从 %DLQ%{group} 消费到。
func TestDLQ(t *testing.T) {
	...
	p := newProducer(t, topic)
	sendOne(t, p, topic, body)

	c := newConsumer(t, topic, group)
	// 收 2 次不 ack：受服务端退避下限（第 2 次投递后 10s 不可见）影响，
	// 每轮收到后等待「不可见窗口 + 余量」再继续
	seen := 0
	deadline := time.Now().Add(120 * time.Second)
	for seen < 2 && time.Now().Before(deadline) {
		mvs, err := c.Receive(context.TODO(), 16, invisibleWindow)
		if err != nil {
			continue // 空轮询错误码是 SDK 正常行为
		}
		for _, mv := range mvs {
	...
	// 等第 2 次投递的不可见窗口（max(invisibleWindow, 服务端退避 10s)）过期，
	// 下一次 Receive 触发 DLQ 转入
	time.Sleep(11 * time.Second)

	// 从 %DLQ%{group} 消费死信（普通 topic，SDK 直接订阅）
	dlqTopic := "%DLQ%" + group
	dc := newConsumer(t, dlqTopic, group+"-dlqreader")
	// 原 topic 的 Receive 触发转入动作（惰性：无人收就无人转）。
	// SDK 按队列轮询，单次 Receive 可能落在别的队列——戳满 4 次覆盖全部队列
	for i := 0; i < 4; i++ {
		_, _ = c.Receive(context.TODO(), 16, invisibleWindow) // 空轮询错误可忽略
	}
	got := receiveBodies(t, dc, 1, 20*time.Second)
	if len(got) != 1 || got[0] != body {
		t.Fatalf("死信未到达: %v", got)
	}
}
```
...
Run: `make e2e`
Expected: 既有 4 用例 + 新 2 用例全部 PASS。排查顺序（沿用 M1 经验）：先看 broker 日志（测试失败时自动打印），再看 SDK console 日志；`NewFilterExpression` 构造名/`SetTag` 方法名与 SDK 版本不符时以 `$GOMODCACHE/github.com/apache/rocketmq-clients/golang/v5@v5.1.4/` 下源码为准。
...
**3. 类型/签名一致性：**
- `meta.New(st, autoCreate, defaultQueues uint32, defaultMaxAttempts int32, logger)`：Task 3 定义，Task 3 内更新全部 4 个调用点，Task 4/6/8 的新 fixture 均按此签名 ✓
...
- config 新增 4 字段与「任务间接口总表」一致（Task 1 校验、Task 3 `default_max_attempts`、Task 8 `retention_check_interval`、Task 9 `disk_watermark_percent`）✓
