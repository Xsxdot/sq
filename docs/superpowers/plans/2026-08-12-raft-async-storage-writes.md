...
> **2026-08-12 修订**：该线按字面**未通过**——补上 append 合批后 mem 档实测
> −4.5%/−4.5%/−3.3%（跨轮复现）。这条线是在 §7 预期 mem 档小幅**收益**的前提下
> 写的，没有预料到真实结果是「用 mem 档 4.5% 换 fsync 档 +38.3%」这种跨档位交易。
> 经确认接受该交易，本线修订为「**mem 档劣化不超过 5%**」，据此判定通过。
> 修订仅适用于本次；那 4.5% 是 AsyncStorageWrites 的分发税，与合批无关，
> 后续若要拿回需单独攻分发开销。详见设计文档状态段「第二次实测」。
