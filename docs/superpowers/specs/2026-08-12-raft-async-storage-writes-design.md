...
日期：2026-08-12
状态：**已实测两轮——本方案单独落地全面劣化；补上 append 阶段 group commit 后
fsync 档 +38.3%，以「mem 档劣化不超过 5%」的修订验收线合入**（2026-08-12 三机实测）

> **第二次实测（补丁 `23c3645`，同批机器 main=`1b2e680` vs gc=`23c3645`，轮内交错 3 轮，40 格无一异常）**
>
> `runAppend` 改为批量抽干 `appendCh`、一批内各条 `Persist(sync=false)` 后合成
> **一次** `seglog.Log.Sync()` 再统一投递 Responses——也就是下面那段「未实现、
> 未验证」的修正。结果：
>
> | 档位 | ack | conc | main | gc | 比值中位 |
> |---|---|---|---|---|---|
> | C1 | fsync | 16 | 2078 | 2896 | +37.1% |
> | C1 | fsync | 256 | 4947 | 6897 | +39.4% |
> | C1 | mem | 16 | 8581 | 8191 | −4.5% |
> | C1 | mem | 256 | 11140 | 10917 | 持平 |
> | C3cross | fsync | 16 | 2469 | 3282 | +32.9% |
> | C3cross | fsync | 256 | 3622 | 5229 | +43.9% |
> | C3cross | mem | 16 | 8070 | 7816 | −4.5% |
> | C3cross | mem | 256 | 10667 | 10191 | −3.3% |
>
> fsync 档配对比值中位数 **1.383（+38.3%）**，12/12 轮同向。摊薄比（同样用
> `/proc/diskstats` 直接数 flush）全面回升：C1/16 1.10→1.67，
> **C1/256 3.94→7.30（×1.85）**，C3cross/16 1.49→2.19，C3cross/256 2.52→4.06。
> 三副本 flush 次数同步下降，不是把写挪到了别的机器上。吞吐增幅与摊薄比增幅
> 逐格同序——与第一次实测里「摊薄比跌得越狠、吞吐掉得越多」是同一条相关性，
> 方向相反。
>
> **两个必须一并记住的代价**：
>
> 1. **mem 档三格劣化 −4.5%/−4.5%/−3.3%**（跨轮复现，波动 <1pt）。这是
>    AsyncStorageWrites 的**分发税**本身：mem 档不走 fsync 路径，合批永远不
>    触发，既不加剧也无从消除。计划原定「劣化即失败」的硬底线**按字面未通过**；
>    该线是在本文 §7 预期 mem 档小幅**收益**的前提下写的，没有预料到会出现
>    「用 mem 档 4.5% 换 fsync 档 38%」这种跨档位交易。经确认接受该交易，
>    验收线修订为「**mem 档劣化不超过 5%**」。要真正拿回这 4.5%，只能去砍
>    分发开销（例如 mem 模式走不入 `appendCh` 的快路径），是另一件事。
> 2. **fsync 档高并发尾延迟变差**：C3cross/256 p99 148→244ms，而 p50 63→11ms。
>    group commit 的固有形状——多数请求搭批次便车，批头那个要等整批攒齐。
>    conc=16 时 p99 反而改善（16.7→13.8ms）。
>
> 离散度：仅 C3cross/fsync/256 的 gc 侧达 20.6%，该格**绝对值不可引用**，
> 配对比值仍可用。原始数据：会话 scratchpad `attribution/rawC/`。

> **第一次实测（Task 6，同批机器 main=`1b2e680` vs async=`1f002b3`，轮内交错 3 轮）**
>
