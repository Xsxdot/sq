...
> **修订始末（2026-08-11，三行定案最终全部维持）**：第三行「每组独立日志」
> 曾被当日上午的三机实测推翻——fsync 档高并发回退 47%（conc=256：2432 vs
> main 4589 msg/s）。据此设计并实现了全组共享单段文件链 + sync 领导者
> group commit（`2026-08-11-raftlog-shared-seglog-design.md`，分支
> `v2-sharedlog-gc`），**但该修订已被当晚的复测否决**：共享方案在 fsync 档
> 每个并发档都劣于每组独立 15-17%（成本在跨组 fsync 被串成一列，而每组独立
> 时 4 个组可并行发 fsync）。
>
> 同时暴露出：上午那批 -47% 的数字与晚间重拉的测试机不可比——同样的 seglog
> 在新机器上只回退 9%，低并发反而领先，新机器上 main 的 fsync 吞吐甚至高于
> mem（物理上不可能，说明该虚拟盘 fsync 近乎免费）。**「fsync 档回退幅度
> 强烈依赖磁盘特性」是本表第三行的已知限制，但不足以推翻它。**
>
> 因此**本表三行定案全部维持原样**，共享方案的代码保留待命（否决理由与
> 复活条件见共享 spec 顶部的「否决结论」一节）。
