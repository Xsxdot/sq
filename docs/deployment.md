...
三节点集群在三台都启动之前起不来，这是预期行为：启动过程会阻塞等待元数据组选出 leader，凑不齐多数派就等不到，60 秒后超时退出（半残启动比拒绝启动更糟——所有元数据写路径都会失败）。systemd 的 `Restart=on-failure` 会每 5 秒重试一次，等另外两台起来后自然转为正常。

这期间该节点的管理面尚未监听，`sq status` 报的是**退出码 1（够不着本机管理面）**，不是 2。退出码 2 表示管理面已经能回答问题、但集群确实降级了，两者不要混淆。
